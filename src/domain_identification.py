...
from common import module_results, path, subprocessing, utils
from common.fasta import get_fasta_from_features
from common.hmmscan_refinement import refine_hmmscan_results, HMMResult
from common.secmet.record import Record
from common.secmet.features import (
    AntismashDomain,
    CDSFeature,
    CDSMotif,
    Module as ModuleFeature,
)
from common.secmet.locations import FeatureLocation
from common.config import get_config

from common.module_identification import (
    build_modules_for_cds,
    CDSModuleInfo,
    combine_modules,
    Module,
)
from common.modular_domain import ModularDomain
...
class NRPSPKSDomains(module_results.DetectionResults):
    ...
    def to_json(self) -> Dict[str, Any]:
        return {
            "cds_results": {
                cds.get_name(): cds_result.to_json()
                for cds, cds_result in self.cds_results.items()
            },
...
def get_database_path(subdir: str, filename: str) -> str:
    ...
    if subdir not in DATABASE_PATHS:
        ...
        latest = path.find_latest_database_version(
            database_root, required_file_pattern=filename
        )
...
def _strip_trailing_numbers(hit: HMMResult) -> HMMResult:
    """Removes the trailing numbers from profile names with numeric
       identifiers (e.g. ST_2, ST_3).
    """
    name = hit.hit_id
...
def generate_domains(record: Record) -> NRPSPKSDomains:
    """Annotates NRPS/PKS domains on CDS features."""
    results = NRPSPKSDomains(record.id)

    cds_within_regions = record.get_cds_features_within_regions()
    assert cds_within_regions, "No genes found in regions"

    fasta = get_fasta_from_features(cds_within_regions)
    cds_domains = find_domains(fasta, record)
    cds_ks_subtypes = find_subtypes(
        "PKS_KS",
        path.get_full_path(__file__, "data", "ksdomains.hmm"),
        cds_domains,
        record,
    )
    find_subtypes(
        "Trans-AT-KS",
        os.path.join(get_database_path("transATor", "transATor.hmm")),
        cds_ks_subtypes,
        record,
        modifier_callback=_strip_trailing_numbers,
    )
    ...
    for cds, cds_result in results.cds_results.items():
        # Filter out modules with only a single component
        cds_result.modules = list(
            filter(lambda mod: len(mod.components) > 1, cds_result.modules)
        )
        cds_result.annotate_domains(record, cds)
    return results


def filter_nonterminal_docking_domains(
    record: Record, cds_domains: Dict[str, List[HMMResult]]
) -> Dict[str, List[HMMResult]]:
    """For multiprotein domains, remove docking-terminal predictions that
       are not overlapping with the N- or C-terminal 50 aa of the protein.
    """
    dockingdomains = {
        'NRPS-COM_Nterm',
        'NRPS-COM_Cterm',
    ...
    for cds_name in list(cds_domains):
        ...
        for hit in cds_domains[cds_name]:
            if (
                hit.hit_id in dockingdomains
                and not (
                    cds_length - max(hit.query_start, hit.query_end) < 50
...
def find_subtypes(
    target: str,
    database_path: str,
    existing_domains: Dict[str, List[HMMResult]],
    record: Record,
    modifier_callback: Callable[[HMMResult], HMMResult] = None,
    options: List[str] = None,
) -> Dict[str, List[HMMResult]]:
    """Finds subtypes of the given target from the existing hits."""
    relevant_features = []
    ...
    for cds_name, all_hits in new_domains.items():
        filtered = []
        relevant_domains = [
            hit for hit in existing_domains[cds_name] if hit.hit_id == target
        ]
...
def generate_domain_features(
    gene: CDSFeature, domains: List[HMMResult]
) -> Dict[HMMResult, ModularDomain]:
    ...
    for domain in domains:
        ...
        # Set up new feature
        new_feature = ModularDomain(
            loc, protein_location=prot_loc, locus_tag=gene.get_name()
        )
        name = domain.hit_id
...
def generate_motif_features(feature: CDSFeature, motifs: List[HMMResult]) -> List[CDSMotif]:
    ...
    for i, motif in enumerate(motifs):
        ...
        new_motif = CDSMotif(
            loc, feature.get_name(), prot_loc, tool="nrps_pks_domains"
        )
...
# ======================= EXTRA SECTION FOR FASTA SCAN =========================
if __name__ == "__main__":
    import argparse
    from Bio import SeqIO  # Biopython

    parser = argparse.ArgumentParser(
        description="Scan protein sequences with nrpspksdomains HMM and output domain CSV."
    )
    parser.add_argument(
        "-r", "--ref",
        required=True,
        help="HMM database file (e.g. nrpspksdomains.hmm).",
    )
    parser.add_argument(
        "-f", "--faa",
        required=True,
    ...
    for fpath in [hmm_file, fasta_file]:
        if not os.path.exists(fpath):
            sys.exit(f"[ERROR] File does not exist: {fpath}")

    print(f"[INFO] Database: {hmm_file}")
    print(f"[INFO] Scanning sequences: {fasta_file}")

    opts = ["--cut_tc"]

    # Run hmmscan
    print("[INFO] Running hmmscan ...")
    with open(fasta_file, "r", encoding="utf-8") as handle:
        fasta_content = handle.read().strip()

    results_raw = subprocessing.run_hmmscan(hmm_file, fasta_content, opts)

    # Parse and refine hmmscan results
    print("[INFO] Analysing results ...")
    lengths = utils.get_hmm_lengths(hmm_file)
    refined = refine_hmmscan_results(results_raw, lengths, neighbour_mode=True)

    # Build mapping from Seq_ID to protein sequence using Biopython
    seq_to_record: Dict[str, "SeqRecord"] = {}
    with open(fasta_file, "r", encoding="utf-8") as handle:
        for rec in SeqIO.parse(handle, "fasta"):
            # rec.id is the first token in the header, usually matching hmmscan query name
            seq_to_record[rec.id] = rec

    # Write CSV
    print(f"[INFO] Writing results to {csv_file} ...")
    with open(csv_file, "w", newline="", encoding="utf-8") as handle:
        ...
        writer.writerow(
            [
                ...
                "Range End",
                "domain_sequence",
                "Order",  # domain order per sequence
            ]
        )

        any_hit = False

        # For each sequence, sort hits by start position and number them
        for seqname, hits in refined.items():
            rec = seq_to_record.get(seqname)

            hits_sorted = sorted(hits, key=lambda h: min(h.query_start, h.query_end))
            order = 0
            for h in hits_sorted:
                any_hit = True
                order += 1

                start = min(h.query_start, h.query_end)
                end = max(h.query_start, h.query_end)

                # hmmscan: 1-based inclusive; Biopython slice: 0-based, end-excluded
                domain_seq_str = ""
                if rec is not None:
                    start_idx = max(start - 1, 0)
                    end_idx = end
                    domain_seq = rec.seq[start_idx:end_idx]
                    domain_seq_str = str(domain_seq)
    ...
    print("\n[RESULT] NRPS/PKS domain identification results:")
    print(
        f"{'Seq_ID':20s}\t{'Domain':25s}\t{'E-value':>10s}\t"
        f"{'Bitscore':>10s}\t{'Range':>10s}"
    )
    ...
    for seqname, hits in refined.items():
        for h in hits:
            ...
            print(
                f"{seqname:20s}\t{h.hit_id:25s}\t"
                f"{h.evalue:10.2e}\t{h.bitscore:10.1f}\t"
                f"{start}-{end}"
            )

    if any_hit:
        print(f"\n[INFO] Scan finished. Results saved as: {csv_file}")
    else:
        print("[INFO] No NRPS/PKS domains were detected.")
# ==============================================================================